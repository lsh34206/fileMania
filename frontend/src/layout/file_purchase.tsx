...
function FilePurchase() {
  ...
  const buy = async () => {
    setSubmitting(true)
    try {
      const res = await axios.post(import.meta.env.VITE_API_VALUE+`/purchase/${type}/${id}`, {}, { withCredentials: true })
      if (res.data.success) {
      ...
      <div className='card'>
        {(type === 'image' || type === 'video') && (
          <div className='purchase-preview'>
            <img
              src={import.meta.env.VITE_BACKEND_URI_VALUE+`/media/${type}/${id}/preview`}
              alt={data.file.title}
