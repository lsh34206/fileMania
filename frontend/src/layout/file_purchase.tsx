...
function FilePurchase() {
    ...
    const load = async () => {
      ...
      try {
        const [viewRes, meRes] = await Promise.all([
          axios.get(`http://localhost:8080/download/paid/${type}/${id}`, { withCredentials: true }),
  ...
  const buy = async () => {
    setSubmitting(true)
    try {
      const res = await axios.post(`http://localhost:8080/purchase/${type}/${id}`, {}, { withCredentials: true })
      if (res.data.success) {
      ...
      <div className='card'>
        {(type === 'image' || type === 'video') && (
          <div className='purchase-preview'>
            <img
              src={`http://localhost:8080/media/${type}/${id}/preview`}
              alt={data.file.title}
